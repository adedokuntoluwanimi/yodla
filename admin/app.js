...
const imageOf = (document) => document.imageUrl || document.previewImageUrl || (typeof document.image === "string" ? document.image : document.legacyImage) || "";
...
function renderDashboard() {
  ...
  const persistNote = state.persist?.durable
    ...
    : "This host cannot keep publishes yet. Add BLOB_READ_WRITE_TOKEN on Vercel so the shelf survives.";
...
function toolbar(noun) {
  return `<div class="content-toolbar"><input type="search" data-content-search value="${escapeHtml(state.query)}" placeholder="Search ${noun}s" aria-label="Search ${noun}s"><select data-status-filter aria-label="Filter by publishing status"><option value="all">All statuses</option><option value="published" ${state.filter === "published" ? "selected" : ""}>Published</option><option value="modified" ${state.filter === "modified" ? "selected" : ""}>Modified</option><option value="draft" ${state.filter === "draft" ? "selected" : ""}>Draft only</option></select></div>`;
}
...
function formDocument(form) {
  ...
  if (data._type === "homePage") Object.assign(document, { hero: { eyebrow: data.heroEyebrow, title: data.heroTitle, description: data.heroDescription, primaryLabel: data.heroPrimaryLabel, primaryHref: data.heroPrimaryHref, imageProductId: data.heroImageProductId, imageAlt: data.heroImageAlt, caption: data.heroCaption, captionStrong: data.heroCaptionStrong, stamp: data.heroStamp }, featuredProductIds: split(data.featuredProductIds), occasions: linePairs(data.occasions, ["key", "eyebrow", "title", "action"]), journalPromotionId: data.journalPromotionId, manifesto: { eyebrow: data.manifestoEyebrow, title: data.manifestoTitle, description: data.manifestoDescription, steps: linePairs(data.manifestoSteps, ["title", "body"]) } });
  return document;
}
...
async function load(initial = true) {
  try {
    ...
    if (initial) status(state.persist.durable ? "Content shelf is ready." : "Shelf loaded. Publishes will not persist on Vercel until a blob token is set.", state.persist.durable ? "success" : "");
    render();
  } catch (error) {
