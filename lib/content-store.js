...
export async function ensureLoaded({ force = false } = {}) {
  ...
  if (loaded && !force && !(process.env.VERCEL && stale)) return;
  const saved = await loadShelf();
  if (saved?.records) hydrate(saved);
  else seedFromFallback();
  loaded = true;
  loadedAt = Date.now();
}
...
function view(group) {
  ...
  return {
    ...document,
    _baseId: group.id,
    _status: group.draft ? (group.published ? "modified" : "draft") : "published",
...
export async function saveDocument(document, action) {
  ...
  if (action === "save") current.draft = stamped;
    ...
    current.published = { ...stamped, publishedAt: stamped.publishedAt || now() };
    current.draft = null;
...
export async function getPublishedSnapshot() {
  await ensureLoaded({ force: Boolean(process.env.VERCEL) });
